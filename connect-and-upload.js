    ...
    filesOk.forEach(function(file){
      ...
      reader.onloadend = function() {
          ipfsRequest (file.name, buffer.Buffer(reader.result)).then((data) => {
            ...
            updateList(fileChecksum(file), data[0].hash, data[0].path)
            uploadCount++
...
function updateList (checksum, ipfsHash, fileName) {
  ...
  var hashToUpload = ipfsHash;
  var linkToUpload = 'http://127.0.0.1:8080/ipfs/'+ipfsHash;
  var fileName = fileName
  let data = {  id: "15254588777",
  sender: "2001",
  receiver: "2002",
  fileHash: hashToUpload,
  fileLink: linkToUpload,
fileName: fileName};

fetch("http://localhost:3000/createNewFileTransfer", {
  method: "POST",
...
function getFilesList() {
  ...
  var files 
   ...
   fetch('http://localhost:3000/getFilesList?id=2002')
  .then((response) => response.json())
  .then((data) => {files = data}).then(() => {console.log(files);
  let list = document.getElementById("receiveId");
  //create new li element

files.forEach((item)=>{
  if(item.data.newTransaction != null && item.data.newTransaction.fileHash != null){
  let li = document.createElement("li");
  var text = item.data.newTransaction.fileName;
  var link =  item.data.newTransaction.fileLink;
  li.innerHTML = text.link(link);
  li.download = text;
  list.appendChild(li);
  li.style.padding = '10px';
  li.style.borderWidth = '5px';
...
//         // files_checksum[filesOk.length] = fileChecksum(files[i])
//         document.querySelector("#receivelist").querySelector("ul").innerHTML += '<li id="' + files[i].data.newTransaction.receiver + '"><strong class="fileName">' +
//         files[i].name + '</strong> <spam class="itemClose"><a class="removeItem" href="#" onclick="removeItem(\''+files[i].data.newTransaction.receiver+'\')">&times;</a></spam>' +
// '</spam></li>';
